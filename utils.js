...
function formatCurrency(amount) {
    ...
    if (isNaN(parsedAmount)) return `A$ 0,00`;
    return new Intl.NumberFormat('pt-AO', { style: 'currency', currency: 'AOA' }).format(parsedAmount).replace('AOA', 'A$');
}
...
function createModal(title, content, buttons = []) {
    ...
    modalOverlay.innerHTML = `
        <div class="glass-panel modal-content-wrapper">
            ...
            <div class="modal-actions">${buttons.map(btn => `<button class="btn ${btn.class}" onclick="${btn.onclick}"><i data-lucide="${btn.icon}"></i> ${btn.text}</button>`).join('')}</div>
        </div>
    `;
...
function getGeminiResponse(prompt) {
    ...
    if (p.includes('jogo') || p.includes('quiz')) return "Na secção 'Jogos & Recompensas', você pode participar do Quiz Financeiro e ganhar até A$ 1.875,00 em saldo de jogos! É divertido e educativo.";
